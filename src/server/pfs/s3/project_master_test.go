...
func projectMasterListBuckets(ctx context.Context, t *testing.T, pachClient *client.APIClient, minioClient *minio.Client) {
...
func projectMasterListBucketsBranchless(ctx context.Context, t *testing.T, pachClient *client.APIClient, minioClient *minio.Client) {
...
func projectMasterGetObject(ctx context.Context, t *testing.T, pachClient *client.APIClient, minioClient *minio.Client) {
	...
	fetchedContent, err := getObject(ctx, t, minioClient, fmt.Sprintf("master.%s.%s", repo, pfs.DefaultProjectName), "file")
	require.NoError(t, err)
	require.Equal(t, "content", fetchedContent)
}

func projectMasterGetObjectInBranch(ctx context.Context, t *testing.T, pachClient *client.APIClient, minioClient *minio.Client) {
	...
	fetchedContent, err := getObject(ctx, t, minioClient, fmt.Sprintf("branch.%s.%s", repo, pfs.DefaultProjectName), "file")
	require.NoError(t, err)
	require.Equal(t, "content", fetchedContent)
}

func projectMasterStatObject(ctx context.Context, t *testing.T, pachClient *client.APIClient, minioClient *minio.Client) {
	...
	info, err := minioClient.StatObject(ctx, fmt.Sprintf("master.%s.%s", repo, pfs.DefaultProjectName), "file", minio.StatObjectOptions{})
	require.NoError(t, err)
...
func projectMasterPutObject(ctx context.Context, t *testing.T, pachClient *client.APIClient, minioClient *minio.Client) {
	repo := tu.UniqueString("testputobject")
	require.NoError(t, pachClient.CreateRepo(pfs.DefaultProjectName, repo))
	require.NoError(t, pachClient.CreateBranch(pfs.DefaultProjectName, repo, "branch", "", "", nil))

	r := strings.NewReader("content1")
	_, err := minioClient.PutObject(ctx, fmt.Sprintf("branch.%s.%s", repo, pfs.DefaultProjectName), "file", r, int64(r.Len()), minio.PutObjectOptions{ContentType: "text/plain"})
	require.NoError(t, err)

	// this should act as a PFS PutFile
	r2 := strings.NewReader("content2")
	_, err = minioClient.PutObject(ctx, fmt.Sprintf("branch.%s.%s", repo, pfs.DefaultProjectName), "file", r2, int64(r2.Len()), minio.PutObjectOptions{ContentType: "text/plain"})
	require.NoError(t, err)
...
func projectMasterRemoveObject(ctx context.Context, t *testing.T, pachClient *client.APIClient, minioClient *minio.Client) {
	...
	require.NoError(t, minioClient.RemoveObject(ctx, fmt.Sprintf("master.%s.%s", repo, pfs.DefaultProjectName), "file", minio.RemoveObjectOptions{}))
	require.NoError(t, minioClient.RemoveObject(ctx, fmt.Sprintf("master.%s.%s", repo, pfs.DefaultProjectName), "file", minio.RemoveObjectOptions{}))

	// make sure the object no longer exists
	_, err := getObject(ctx, t, minioClient, fmt.Sprintf("master.%s.%s", repo, pfs.DefaultProjectName), "file")
	keyNotFoundError(t, err)
}

// Tests inserting and getting files over 64mb in size
func projectMasterLargeObjects(ctx context.Context, t *testing.T, pachClient *client.APIClient, minioClient *minio.Client) {
	...
	// error
	_, err = minioClient.FPutObject(ctx, fmt.Sprintf("master.%s.%s", repo2, pfs.DefaultProjectName), "file", inputFile.Name(), minio.PutObjectOptions{
		ContentType: "text/plain",
	})
	bucketNotFoundError(t, err)

	// now try putting into a legit repo
	l, err := minioClient.FPutObject(ctx, fmt.Sprintf("master.%s.%s", repo1, pfs.DefaultProjectName), "file", inputFile.Name(), minio.PutObjectOptions{
		ContentType: "text/plain",
	})
	require.NoError(t, err)
	...
	err = minioClient.FGetObject(ctx, fmt.Sprintf("master.%s.%s", repo2, pfs.DefaultProjectName), "file", "foo", minio.GetObjectOptions{})
	bucketNotFoundError(t, err)
	...
	err = minioClient.FGetObject(ctx, fmt.Sprintf("master.%s.%s", repo1, pfs.DefaultProjectName), "file", outputFile.Name(), minio.GetObjectOptions{})
...
func projectMasterGetObjectNoHead(ctx context.Context, t *testing.T, pachClient *client.APIClient, minioClient *minio.Client) {
	repo := tu.UniqueString("testgetobjectnohead")
	require.NoError(t, pachClient.CreateRepo(pfs.DefaultProjectName, repo))
	require.NoError(t, pachClient.CreateBranch(pfs.DefaultProjectName, repo, "branch", "", "", nil))

	_, err := getObject(ctx, t, minioClient, fmt.Sprintf("branch.%s.%s", repo, pfs.DefaultProjectName), "file")
	keyNotFoundError(t, err)
}

func projectMasterGetObjectNoBranch(ctx context.Context, t *testing.T, pachClient *client.APIClient, minioClient *minio.Client) {
...
func projectMasterGetObjectNoRepo(ctx context.Context, t *testing.T, pachClient *client.APIClient, minioClient *minio.Client) {
	repo := tu.UniqueString("testgetobjectnorepo")
	_, err := getObject(ctx, t, minioClient, fmt.Sprintf("master.%s.%s", repo, pfs.DefaultProjectName), "file")
	bucketNotFoundError(t, err)
}

func projectMasterMakeBucket(ctx context.Context, t *testing.T, pachClient *client.APIClient, minioClient *minio.Client) {
	repo := tu.UniqueString("testmakebucket")
	require.NoError(t, minioClient.MakeBucket(ctx, fmt.Sprintf("master.%s.%s", repo, pfs.DefaultProjectName), minio.MakeBucketOptions{}))
...
func projectMasterMakeBucketWithBranch(ctx context.Context, t *testing.T, pachClient *client.APIClient, minioClient *minio.Client) {
	repo := tu.UniqueString("testmakebucketwithbranch")
	require.NoError(t, minioClient.MakeBucket(ctx, fmt.Sprintf("branch.%s.%s", repo, pfs.DefaultProjectName), minio.MakeBucketOptions{}))
...
func projectMasterMakeBucketWithRegion(ctx context.Context, t *testing.T, pachClient *client.APIClient, minioClient *minio.Client) {
	repo := tu.UniqueString("testmakebucketwithregion")
	require.NoError(t, minioClient.MakeBucket(ctx, fmt.Sprintf("master.%s.%s", repo, pfs.DefaultProjectName), minio.MakeBucketOptions{Region: "us-east-1"}))
	_, err := pachClient.InspectRepo(pfs.DefaultProjectName, repo)
	require.NoError(t, err)
}

func projectMasterMakeBucketRedundant(ctx context.Context, t *testing.T, pachClient *client.APIClient, minioClient *minio.Client) {
	repo := tu.UniqueString("testmakebucketredundant")
	require.NoError(t, minioClient.MakeBucket(ctx, fmt.Sprintf("master.%s.%s", repo, pfs.DefaultProjectName), minio.MakeBucketOptions{}))
	err := minioClient.MakeBucket(ctx, fmt.Sprintf("master.%s.%s", repo, pfs.DefaultProjectName), minio.MakeBucketOptions{})
	require.YesError(t, err)
	require.Equal(t, err.Error(), "The bucket you tried to create already exists, and you own it.")
}

func projectMasterMakeBucketDifferentBranches(ctx context.Context, t *testing.T, pachClient *client.APIClient, minioClient *minio.Client) {
	repo := tu.UniqueString("testmakebucketdifferentbranches")
	require.NoError(t, minioClient.MakeBucket(ctx, fmt.Sprintf("master.%s.%s", repo, pfs.DefaultProjectName), minio.MakeBucketOptions{}))
	require.NoError(t, minioClient.MakeBucket(ctx, fmt.Sprintf("branch.%s.%s", repo, pfs.DefaultProjectName), minio.MakeBucketOptions{}))
}

func projectMasterBucketExists(ctx context.Context, t *testing.T, pachClient *client.APIClient, minioClient *minio.Client) {
	repo := tu.UniqueString("testbucketexists")

	exists, err := minioClient.BucketExists(ctx, fmt.Sprintf("master.%s.%s", repo, pfs.DefaultProjectName))
	require.NoError(t, err)
	...
	// repo exists, but branch doesn't: should be false
	exists, err = minioClient.BucketExists(ctx, fmt.Sprintf("branch.%s.%s", repo, pfs.DefaultProjectName))
	require.NoError(t, err)
	require.False(t, exists)
...
func projectMasterRemoveBucket(ctx context.Context, t *testing.T, pachClient *client.APIClient, minioClient *minio.Client) {
	...
	require.NoError(t, minioClient.RemoveBucket(ctx, fmt.Sprintf("master.%s.%s", repo, pfs.DefaultProjectName)))
	require.NoError(t, minioClient.RemoveBucket(ctx, fmt.Sprintf("branch.%s.%s", repo, pfs.DefaultProjectName)))
}

func projectMasterRemoveBucketBranchless(ctx context.Context, t *testing.T, pachClient *client.APIClient, minioClient *minio.Client) {
	repo := tu.UniqueString("testremovebucketbranchless")

	// should error out because the repo doesn't have a branch
	require.NoError(t, pachClient.CreateRepo(pfs.DefaultProjectName, repo))
	bucketNotFoundError(t, minioClient.RemoveBucket(ctx, fmt.Sprintf("master.%s.%s", repo, pfs.DefaultProjectName)))
}

func projectMasterListObjectsPaginated(ctx context.Context, t *testing.T, pachClient *client.APIClient, minioClient *minio.Client) {
	...
	ch := minioClient.ListObjects(ctx, fmt.Sprintf("master.%s.%s", repo, pfs.DefaultProjectName), minio.ListObjectsOptions{UseV1: true})
	...
	ch = minioClient.ListObjects(ctx, fmt.Sprintf("%s.%s.%s.%s", commit.Id, "master", repo, pfs.DefaultProjectName), minio.ListObjectsOptions{UseV1: true})
	checkListObjects(t, ch, &startTime, &endTime, expectedFiles, []string{"dir/"})

	// Request that will list all files in master starting with 1
	ch = minioClient.ListObjects(ctx, fmt.Sprintf("master.%s.%s", repo, pfs.DefaultProjectName), minio.ListObjectsOptions{UseV1: true, Prefix: "1"})
	expectedFiles = []string{}
	for i := 0; i <= 1000; i++ {
	...
	ch = minioClient.ListObjects(ctx, fmt.Sprintf("master.%s.%s", repo, pfs.DefaultProjectName), minio.ListObjectsOptions{UseV1: true, Prefix: "dir/"})
	expectedFiles = []string{}
	for i := 0; i < 10; i++ {
...
func projectMasterListObjectsHeadlessBranch(ctx context.Context, t *testing.T, pachClient *client.APIClient, minioClient *minio.Client) {
	...
	ch := minioClient.ListObjects(ctx, fmt.Sprintf("emptybranch.%s.%s", repo, pfs.DefaultProjectName), minio.ListObjectsOptions{UseV1: true})
	checkListObjects(t, ch, nil, nil, []string{}, []string{})
}

func projectMasterListObjectsRecursive(ctx context.Context, t *testing.T, pachClient *client.APIClient, minioClient *minio.Client) {
	// `startTime` and `endTime` will be used to ensure that an object's
	// `LastModified` date is correct. A few minutes are subtracted/added to
	// each to tolerate the node time not being the same as the host time.
	startTime := time.Now().Add(time.Duration(-5) * time.Minute)
	...
	ch := minioClient.ListObjects(ctx, fmt.Sprintf("master.%s.%s", repo, pfs.DefaultProjectName), minio.ListObjectsOptions{Prefix: "", Recursive: true})
	checkListObjects(t, ch, &startTime, &endTime, expectedFiles, []string{})

	// Requests that will list all files in rootdir
	expectedFiles = []string{"rootdir/1", "rootdir/subdir/2"}
	ch = minioClient.ListObjects(ctx, fmt.Sprintf("master.%s.%s", repo, pfs.DefaultProjectName), minio.ListObjectsOptions{Prefix: "r", Recursive: true})
	checkListObjects(t, ch, &startTime, &endTime, expectedFiles, []string{})
	ch = minioClient.ListObjects(ctx, fmt.Sprintf("master.%s.%s", repo, pfs.DefaultProjectName), minio.ListObjectsOptions{Prefix: "rootdir", Recursive: true})
	checkListObjects(t, ch, &startTime, &endTime, expectedFiles, []string{})
	ch = minioClient.ListObjects(ctx, fmt.Sprintf("master.%s.%s", repo, pfs.DefaultProjectName), minio.ListObjectsOptions{Prefix: "rootdir/", Recursive: true})
	checkListObjects(t, ch, &startTime, &endTime, expectedFiles, []string{})
	...
	ch = minioClient.ListObjects(ctx, fmt.Sprintf("master.%s.%s", repo, pfs.DefaultProjectName), minio.ListObjectsOptions{Prefix: "rootdir/s", Recursive: true})
	checkListObjects(t, ch, &startTime, &endTime, expectedFiles, []string{})
	ch = minioClient.ListObjects(ctx, fmt.Sprintf("master.%s.%s", repo, pfs.DefaultProjectName), minio.ListObjectsOptions{Prefix: "rootdir/subdir", Recursive: true})
	checkListObjects(t, ch, &startTime, &endTime, expectedFiles, []string{})
	ch = minioClient.ListObjects(ctx, fmt.Sprintf("master.%s.%s", repo, pfs.DefaultProjectName), minio.ListObjectsOptions{Prefix: "rootdir/subdir/", Recursive: true})
	checkListObjects(t, ch, &startTime, &endTime, expectedFiles, []string{})
	ch = minioClient.ListObjects(ctx, fmt.Sprintf("master.%s.%s", repo, pfs.DefaultProjectName), minio.ListObjectsOptions{Prefix: "rootdir/subdir/2", Recursive: true})
	checkListObjects(t, ch, &startTime, &endTime, expectedFiles, []string{})
}

func projectMasterDoesNotListSystemRepoBuckets(ctx context.Context, t *testing.T, pachClient *client.APIClient, minioClient *minio.Client) {
...
func TestProjectMasterDriver(t *testing.T) {
	...
	projectTestRunner(env.Context, t, env.PachClient, "master", s3.NewMasterDriver(), func(t *testing.T, pachClient *client.APIClient, minioClient *minio.Client) {
		t.Run("ListBuckets", func(t *testing.T) {
			projectMasterListBuckets(ctx, t, pachClient, minioClient)
		})
		t.Run("ListBucketsBranchless", func(t *testing.T) {
			projectMasterListBucketsBranchless(ctx, t, pachClient, minioClient)
		})
		t.Run("GetObject", func(t *testing.T) {
			projectMasterGetObject(ctx, t, pachClient, minioClient)
		})
		t.Run("GetObjectInBranch", func(t *testing.T) {
			projectMasterGetObjectInBranch(ctx, t, pachClient, minioClient)
		})
		t.Run("StatObject", func(t *testing.T) {
			projectMasterStatObject(ctx, t, pachClient, minioClient)
		})
		t.Run("PutObject", func(t *testing.T) {
			projectMasterPutObject(ctx, t, pachClient, minioClient)
		})
		t.Run("RemoveObject", func(t *testing.T) {
			projectMasterRemoveObject(ctx, t, pachClient, minioClient)
		})
		t.Run("LargeObjects", func(t *testing.T) {
			projectMasterLargeObjects(ctx, t, pachClient, minioClient)
		})
		t.Run("GetObjectNoHead", func(t *testing.T) {
			projectMasterGetObjectNoHead(ctx, t, pachClient, minioClient)
		})
		t.Run("GetObjectNoBranch", func(t *testing.T) {
			projectMasterGetObjectNoBranch(ctx, t, pachClient, minioClient)
		})
		t.Run("GetObjectNoRepo", func(t *testing.T) {
			projectMasterGetObjectNoRepo(ctx, t, pachClient, minioClient)
		})
		t.Run("MakeBucket", func(t *testing.T) {
			projectMasterMakeBucket(ctx, t, pachClient, minioClient)
		})
		t.Run("MakeBucketWithBranch", func(t *testing.T) {
			projectMasterMakeBucketWithBranch(ctx, t, pachClient, minioClient)
		})
		t.Run("MakeBucketWithRegion", func(t *testing.T) {
			projectMasterMakeBucketWithRegion(ctx, t, pachClient, minioClient)
		})
		t.Run("MakeBucketRedundant", func(t *testing.T) {
			projectMasterMakeBucketRedundant(ctx, t, pachClient, minioClient)
		})
		t.Run("MakeBucketDifferentBranches", func(t *testing.T) {
			projectMasterMakeBucketDifferentBranches(ctx, t, pachClient, minioClient)
		})
		t.Run("BucketExists", func(t *testing.T) {
			projectMasterBucketExists(ctx, t, pachClient, minioClient)
		})
		t.Run("RemoveBucket", func(t *testing.T) {
			projectMasterRemoveBucket(ctx, t, pachClient, minioClient)
		})
		t.Run("RemoveBucketBranchless", func(t *testing.T) {
			projectMasterRemoveBucketBranchless(ctx, t, pachClient, minioClient)
		})
		t.Run("ListObjectsPaginated", func(t *testing.T) {
			projectMasterListObjectsPaginated(ctx, t, pachClient, minioClient)
		})
		t.Run("ListObjectsHeadlessBranch", func(t *testing.T) {
			projectMasterListObjectsHeadlessBranch(ctx, t, pachClient, minioClient)
		})
		t.Run("ListObjectsRecursive", func(t *testing.T) {
			projectMasterListObjectsRecursive(ctx, t, pachClient, minioClient)
		})
		t.Run("DoesNotListSystemRepoBucket", func(t *testing.T) {
			projectMasterDoesNotListSystemRepoBuckets(ctx, t, pachClient, minioClient)
		})
	})
}
